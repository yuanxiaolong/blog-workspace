...
2.启动代理，利用 <font color="#9f4074"> cpuguy83/docker-grand-ambassador </font>
...
3.构建slave，还是利用上一篇Blog的Dockerfile，但这次slave启动的时候link的是proxy
...
6.进入slave，发现主从已同步。（之前不引用代理，重启master，slave里是没有新数据的，而且slave的log是找不到old master ip的）
...
7.我们看一下 proxy 的日志，就能发现 master重启后，proxy监听并代理的新的ip。（我之前重启了master 2次，所以ip从 75 -> 97，再 97 -> 106 ）
